    ...
    fn check_rc4_pattern(terms: &TermArena, matches: &mut Vec<CryptoMatch>) {
        ...
        for i in 0..term_count {
            ...
            if matches!(term.op, Op::Store) && term.args.len() == 3 {
                ...
                if let (
                    Op::BvConst { value: ref v_idx, .. },
                    Op::BvConst { value: ref v_val, .. },
        ...
        for i in 0..term_count {
            ...
            let inner_arg = match term.op {
                ...
                Op::BvOr if term.args.len() == 2 => {
                    ...
                    if matches!((&left.op, &right.op), (Op::BvShl, Op::BvLshr) | (Op::BvLshr, Op::BvShl)) {
                        if !left.args.is_empty() && !right.args.is_empty() && left.args[0] == right.args[0] {
                            Some(left.args[0])
                        } else {
                            None
                        }
